...
public class PostApiController {
    ...
    public ResponseEntity<String> addPost(@Validated @RequestBody PostForm obj, BindingResult bindingResult) {
        if(bindingResult.hasErrors()){
            log.info("add-post occurred an validation Error");
            ...
            throw new PostFormException(map);
        }
    ...
    public ResponseEntity<String> editPost(@PathVariable("id") Long id, @RequestBody EditForm obj) {
        ...
        try {
            ...
            e.getStackTrace();
            return new ResponseEntity<>("비밀번호 불일치", BAD_REQUEST);
        }
    ...
    public ResponseEntity<String> pwdCheck(@PathVariable("id") Long id, String password) {
        ...
        else return new ResponseEntity<>("비밀번호 불일치", BAD_REQUEST);
    }
    ...
    public ResponseEntity<String> delComment(@PathVariable("id") Long id, String password) {
        ...
        try {
            ...
            log.info("id = {} , password = {}", id, password);
            e.getStackTrace();
            return new ResponseEntity<>("비밀번호를 다시 확인해주세요.", BAD_REQUEST);
        }
    }
    ...
    public ResponseEntity<String> likePost(@PathVariable("id") Long id, HttpServletRequest req, String operation) {
        ...
        if (operation.equals("add")) {
            try {
                ...
                return new ResponseEntity<>("삭제된 게시글입니다.", BAD_REQUEST);
            }
        ...
        } else return new ResponseEntity<>("잘못된 접근입니다.", BAD_REQUEST);
    }
}
