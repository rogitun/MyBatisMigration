...
public class PostService {
    ...
    public Post getPostWithComments(Long id) {
        ...
        return postWithComments.orElseThrow();
    }

    @Transactional
    public SinglePostDto getPostDto(Post post) {
      //  postRepository.updateView(post.getPostId());
        post.addView();
        return new SinglePostDto(post.getPostId(), post.getTitle(),
                post.getWriter(), post.getContent(),
                post.getCreated(), post.getModified(),
                post.getViewed(), post.getLikes());
    }

    public List<String> makeHashTags(List<HashtagPost> hashtags) {
        List<String> collect = hashtags.stream().map(h -> h.getHashtag().getName()).collect(Collectors.toList());
        return collect;
    }
}
